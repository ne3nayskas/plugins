function startPlugin() {  
    ...
    // Додаємо контент на головний екран - ПЕРШИЙ канал з перевіркою готовності Favorite  
    if(Lampa.Manifest.app_digital >= 300){  
        // Чекаємо на готовність системи Favorite  
        function addContentRows(){  
            Lampa.ContentRows.add({  
                ...
                screen: ['main'],  
                call: (params, screen)=>{  
                    // Перевіряємо чи увімкнено відображення на головній  
        ...
        // Перевіряємо чи Favorite вже готовий  
        if(Lampa.Favorite && Lampa.Favorite.all().history){  
            addContentRows()  
        } else {  
            // Чекаємо на подію готовності Favorite  
            Lampa.Listener.follow('favorite', (e)=>{  
                if(e.type === 'ready' || e.type === 'update'){  
                    addContentRows()  
                }  
            })  
        }  
