(function () {
  ...
  function Player(station) {
    ...
    // ВИПРАВЛЕННЯ ЗАВДАННЯ 2 (ТВ-бокс): update() дозволяє оновити плеєр на нову
    // станцію без повного знищення/пересворення DOM-елементу, щоб обкладинка
    // праворуч від списку завжди відповідала поточно вибраній станції
    this.update = function (newStation) {
      station = newStation;
      url = station.stream_320 ? station.stream_320 : station.stream_128 ? station.stream_128 : station.stream ? station.stream : station.stream_hls ? station.stream_hls : '';
      stop();

      var cover = html.find('.radio-player__cover');
  ...
  function Component() {
    ...
    // ВИПРАВЛЕННЯ ЗАВДАННЯ 1 і 2:
    // - Додаємо клас radio-content--playing на контейнер списку, який через CSS
    //   розмиває/затемнює фон списку станцій поки грає плеєр (мобільний фікс)
    // - Замість повного знищення/створення нового Player при перемиканні стрілками
    //   (move) використовуємо player.update(), яке оновлює лише дані існуючого
    //   плеєра — це прибирає дублікат обкладинки на ТВ і гарантує що права
    //   обкладинка завжди відповідає поточній станції
    this.play = function (station) {
      played = station;
      var player = new Player(station);
      player.create();
      html.find('.radio-content').addClass('radio-content--playing');
      ...
      var move = function move(d) {
        ...
        if (pos >= 0 && pos < filtred.length) {
          played = filtred[pos];
          player.update(played);
          Lampa.Background.change(played.bg_image_mobile || img_bg);
        }
      };
      ...
      Lampa.Controller.add('content', {
        ...
        back: function back() {
          html.find('.radio-content').removeClass('radio-content--playing');
