...
// Автор оригінального плагіна 
// Плагін створює унікальні підбірки фільмів та серіалів на головній сторінці за жанрами, стрімінговими сервісами, популярністю, переглядами та касовими зборами.  
// Оновлення підбірок відбувається при кожному натисканні кнопки "Головна" (Home).

// ======= Встановлення =======
//  Якщо у вас свій сервер, файл покласти в wwwroot.  
// 1. Для індивідуального використання:  
//    - У Лампа відкрити "Налаштування" → "плагіни".  
//    - У розділі плагінів прописати: ВашАдрес/surs.js.  

// 2. Для завантаження плагіна всім користувачам:  
//    - Додати в lampainit.js рядок:  
//    - Lampa.Utils.putScriptAsync(["/surs.js"], function() {});

// ======= Налаштування =========
//Для заборони користувачу змінювати назву підбірок, використовуйте:
//Lampa.Storage.set('surs_disableCustomName', true); //це приховає пункт меню з вводом власної назви 

//Для встановлення своєї назви для всіх використовуйте:
//Lampa.Storage.set('surs_name', 'YOURS_TITLE');

//Для приховування всього меню "підбірки" використовуйте 
...
var allStreamingServices = [
    ...
    var allStreamingServicesUKR = [
        { id: 3910, title: 'MEGOGO' },
        { id: 3947, title: 'Sweet.tv' },
        { id: 4125, title: '1+1 video' },
        { id: 4135, title: 'ТЕТ' },
        { id: 4145, title: 'СТБ' },
        { id: 4155, title: 'ICTV' },
        { id: 4165, title: 'Новий канал' },
        { id: 4175, title: 'Україна' },
        { id: 4185, title: 'Интер' },
        { id: 4195, title: 'К1' },
        { id: 4205, title: 'К2' }
    ];
...
function getStreamingServicesUKR() {
    return getEnabledItems(allStreamingServicesUKR, 'streaming_ukr_');
}
...
var SourceTMDB = function (parent) {
    ...
    // Оновлюємо функції для підтримки українських джерел
    function getSelectedStreamingServices() {
        var includeGlobal = getStoredSetting('getStreamingServices', true);
        var includeUkrainian = getStoredSetting('getStreamingServicesUKR', true);

        var streamingServices = getStreamingServices();
        var streamingServicesUKR = getStreamingServicesUKR();

        if (includeGlobal && includeUkrainian) {
            return streamingServices.concat(streamingServicesUKR);
        } else if (includeGlobal) {
            return streamingServices;
        } else if (includeUkrainian) {
            return streamingServicesUKR;
        }
        return [];
    }
  ...
  return rusService.id === service.id;
    ...
    // Оновлюємо функцію для українського контенту
        ...
        return function (callback) {
            ...
            owner.get(apiUrl, params, function (json) {
                if (json.results) {
                    ...
                    var titlePrefix = options.ukrainian ? Lampa.Lang.translate('surs_ukrainian') : '';
...
function add() {
    ...
    var surs_mod_ukr = assign({}, Lampa.Api.sources.tmdb, new SourceTMDBukr(Lampa.Api.sources.tmdb));
    ...
    Lampa.Api.sources.surs_mod_ukr = surs_mod_ukr;
    ...
    function startProfileListener() {
        var sourceName = Lampa.Storage.get('surs_name') || 'SURS';
        var sourceNameKids = sourceName + ' KIDS';
        var sourceNameUkr = sourceName + ' UKR'; // Зміна з RUS на UKR
        var sourceNameNew = sourceName + ' NEW';
        ...
        Lampa.Listener.follow('profile', function(event) {
            ...
            } else if (event.params.onlyUkr) { // Зміна з onlyRus на onlyUkr
                changeSource(sourceNameUkr, true);
            } else {
